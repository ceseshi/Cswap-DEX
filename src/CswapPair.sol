...
contract CswapPair is CswapERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, "Cswap: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, "Cswap: FORBIDDEN"); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _update(uint256 balance0, uint256 balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= type(uint112).max && balance1 <= type(uint112).max, "Cswap: OVERFLOW");
